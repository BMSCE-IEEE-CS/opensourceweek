...
async function appendToGoogleSheet(row: unknown[]) {
  const auth = new google.auth.GoogleAuth({
    credentials: {
      client_email: process.env.GOOGLE_SERVICE_ACCOUNT_SCAN_EMAIL!,
      private_key: process.env.GOOGLE_SERVICE_PRIVATE_SCAN_KEY!.replace(
        /\\n/g,
        "\n"
      ),
    },
  ...
  await sheets.spreadsheets.values.append({
    spreadsheetId: process.env.GOOGLE_SHEET_REPOGENESIS_SCAN_ID!,
    range: "Sheet1!A:G",
